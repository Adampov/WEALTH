...
| RISK-005 | Remaining timestamp contracts and persisted JSON or text may preserve a caller offset, including newer stores whose indexed projections are UTC. Mixed textual offsets can also sort incorrectly. | Open / additive canonical foundation complete | Every scoped injected clock now fails unless `tzinfo is datetime.UTC`, before the next ID, HTTP, storage, reservation, wait, log, or canonical-evidence side effect. Pure unused helpers provide strict fixed-UTC validation, explicit aware-input normalization, exact six-digit RFC 3339 `Z` serialization/parsing, and exact integer-only epoch-microsecond projection/decoding across Python's full calendar range. TASK-030 adds exact pinned fingerprints for all eight generated SQLite layouts plus direct immutable read-only inspection, typed marker-byte evidence, source/directory invariants, sidecar rejection, and mutation denial before any timestamp row can be read. Provider/application error mappings and legacy request/model acceptance remain unchanged. No existing model, serializer, query, index, projection, or persistence runtime consumes these foundations; no operator database or timestamp row has been scanned, and most historical bytes remain aware-only. The [TASK-026 inventory and plan](docs/CANONICAL_UTC_BOUNDARY_INVENTORY_AND_MIGRATION_PLAN.md) remains the migration authority. | TASK-031 may add only bounded raw timestamp storage-class and byte evidence from generated fixtures after an exact fingerprint match. Any operator-data preflight still requires approval of the exact path list, immutable snapshot method, report destination, and retention/disposal boundary; compatibility readers, version-2 schemas, quarantine, backup/rollback, and migration verification remain required before global closure. |
