...
| RISK-005 | Remaining timestamp contracts and persisted JSON or text may preserve a caller offset, including newer stores whose indexed projections are UTC. Mixed textual offsets can also sort incorrectly. | Open / additive canonical evidence foundations incomplete | Every scoped injected clock now fails unless `tzinfo is datetime.UTC`, before the next ID, HTTP, storage, reservation, wait, log, or canonical-evidence side effect. Pure unused helpers provide strict fixed-UTC validation, explicit aware-input normalization, exact six-digit RFC 3339 `Z` serialization/parsing, and exact integer-only epoch-microsecond projection/decoding across Python's full calendar range. TASK-030 pins all eight generated SQLite layouts and TASK-031 adds same-connection, exact-snapshot, bounded raw evidence for all 37 direct timestamp columns, including stable row-key bytes, SQLite storage class, exact cast bytes, and length. Mismatch, wrong-family, ambiguity, mutation, sidecars, excess rows, oversized cells, and unstable evidence fail closed. Provider/application mappings and legacy request/model acceptance remain unchanged. No existing model, serializer, query, index, projection, or persistence runtime consumes these foundations; no operator database has been scanned, no timestamp has been interpreted or normalized, and most historical bytes remain aware-only. The [TASK-026 inventory and plan](docs/CANONICAL_UTC_BOUNDARY_INVENTORY_AND_MIGRATION_PLAN.md) remains the migration authority. | TASK-032 may add only a pure typed parser over valid TASK-031 synthetic evidence, preserving original bytes and offsets without SQLite, filesystem, normalization, report, or runtime access. Any operator-data preflight still requires approval of the exact path list, immutable snapshot method, report destination, and retention/disposal boundary; compatibility readers, version-2 schemas, quarantine, backup/rollback, and migration verification remain required before global closure. |
