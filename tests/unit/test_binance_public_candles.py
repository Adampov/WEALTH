...
from wealth.adapters import binance as binance_adapter
...
def test_invalid_timeout_is_rejected_before_http(
    ...
    with pytest.raises(ValueError) as error:
        BinancePublicCandleSource(
            ...
            spot_klines_url="http://must-not-be-validated.test",
            usdm_klines_url="http://must-not-be-validated.test",
        )
...
def test_timeout_policy_uses_shared_exact_maximum_and_default(
    ...
    assert (
        binance_adapter.__dict__["MAX_PUBLIC_HTTP_TIMEOUT_SECONDS"]
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(InstrumentType.SPOT, BINANCE_SPOT_KLINES_URL, id="spot"),
        pytest.param(
