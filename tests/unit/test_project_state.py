...
def test_repository_project_state_is_valid_and_names_one_next_action() -> None:
    ...
    assert state.open_tasks[0].task_id == "TASK-030"
    assert state.open_tasks[0].status == "ready"
    assert state.next_action.task_id == "TASK-030"
    assert state.next_action.action == "phase2.canonical_utc_preflight_fingerprint_foundation"
...
def test_project_state_references_existing_governance_artifacts() -> None:
    ...
    task_029_section = completed_section.split("### TASK-029 ", maxsplit=1)[1].split(
        "### TASK-", maxsplit=1
    )[0]
    ...
    assert "- **Status:** COMPLETE" in task_029_section
    assert "`phase2.canonical_utc_epoch_microsecond_primitives`" in task_029_section
